...
class ApiHelper {
  ...
  static Future<Response> getParcela(Token token, String codEnc) async {
    if (!validToken(token)) {
      return Response(
          isSuccess: false,
          message:
    ...
    var url = Uri.parse('${Constants.apiUrl}/api/Parcelas/$codEnc');
    var response = await http.get(
      url,
      headers: {
        'content-type': 'application/json',
    ...
    var decodedJson = jsonDecode(body);
    return Response(isSuccess: true, result: Parcela.fromJson(decodedJson));
  }
